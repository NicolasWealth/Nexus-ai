...
const HUD_STYLES = `
  ...
  @keyframes glow-emerald {
    0%,100% { box-shadow: 0 0 6px 1px rgba(52,211,153,0.25), inset 0 0 6px rgba(52,211,153,0.05); border-color: rgba(52,211,153,0.35); }
    50%      { box-shadow: 0 0 18px 4px rgba(52,211,153,0.55), inset 0 0 14px rgba(52,211,153,0.12); border-color: rgba(52,211,153,0.8); }
  }
  @keyframes glow-purple {
    0%,100% { box-shadow: 0 0 6px 1px rgba(192,132,252,0.25), inset 0 0 6px rgba(192,132,252,0.05); border-color: rgba(192,132,252,0.35); }
    50%      { box-shadow: 0 0 18px 4px rgba(192,132,252,0.55), inset 0 0 14px rgba(192,132,252,0.12); border-color: rgba(192,132,252,0.8); }
  }
  @keyframes glow-lemon {
    0%,100% { box-shadow: 0 0 6px 1px rgba(232,255,0,0.2), inset 0 0 6px rgba(232,255,0,0.04); border-color: rgba(232,255,0,0.3); }
    50%      { box-shadow: 0 0 18px 4px rgba(232,255,0,0.5), inset 0 0 14px rgba(232,255,0,0.1); border-color: rgba(232,255,0,0.75); }
  }
  @keyframes glow-cyan {
    0%,100% { box-shadow: 0 0 6px 1px rgba(34,211,238,0.2), inset 0 0 6px rgba(34,211,238,0.04); border-color: rgba(34,211,238,0.3); }
    50%      { box-shadow: 0 0 18px 4px rgba(34,211,238,0.5), inset 0 0 14px rgba(34,211,238,0.1); border-color: rgba(34,211,238,0.75); }
  }
  ...
  .animate-flow { animation: flow-bar 2.2s ease-in-out infinite; transform-origin: left; }
  ...
  @keyframes glitch-clip {
    ...
    80% { clip-path: inset(10% 0 70% 0); transform: translate(0, 0); }
  ...
  .corner-tl::before { top: -1px; left: -1px; border-top: 2px solid currentColor; border-left: 2px solid currentColor; border-radius: 3px 0 0 0; }
  .corner-br::after  { bottom: -1px; right: -1px; border-bottom: 2px solid currentColor; border-right: 2px solid currentColor; border-radius: 0 0 3px 0; }
...
// ─── SVG Wireframe patterns (per-card) ────────────────────────────────────────
  ...
  <svg className="absolute inset-0 w-full h-full opacity-[0.07] pointer-events-none" viewBox="0 0 300 180" preserveAspectRatio="xMidYMid slice">
    <defs>
      <pattern id={`circuit-${color}`} width="40" height="40" patternUnits="userSpaceOnUse">
        ...
        <circle cx="40" cy="40" r="1.5" fill={color} opacity="0.5" />
      </pattern>
    </defs>
    ...
    <path d="M10 70 Q75 10 150 70 Q225 130 290 70" stroke={color} strokeWidth="0.4" fill="none" opacity="0.25" />
    <circle cx="150" cy="90" r="20" fill="none" stroke={color} strokeWidth="0.6" strokeDasharray="4 3" opacity="0.4" />
  </svg>
);
...
const WireframeHex = ({ color }: { color: string }) => (
  <svg className="absolute inset-0 w-full h-full opacity-[0.08] pointer-events-none" viewBox="0 0 300 200" preserveAspectRatio="xMidYMid slice">
    ...
    <line x1="60" y1="50" x2="120" y2="130" stroke={color} strokeWidth="0.4" opacity="0.3" />
    <line x1="180" y1="50" x2="120" y2="130" stroke={color} strokeWidth="0.4" opacity="0.3" />
    <line x1="180" y1="50" x2="240" y2="130" stroke={color} strokeWidth="0.4" opacity="0.3" />
    <line x1="300" y1="50" x2="240" y2="130" stroke={color} strokeWidth="0.4" opacity="0.3" />
  </svg>
);
...
const WireframeRadar = ({ color }: { color: string }) => (
  <svg className="absolute inset-0 w-full h-full opacity-[0.07] pointer-events-none" viewBox="0 0 200 200" preserveAspectRatio="xMidYMid slice">
    {[20, 40, 60, 80].map((r) => (
      <circle key={r} cx="100" cy="100" r={r} fill="none" stroke={color} strokeWidth="0.6" />
    ))}
    {[0, 45, 90, 135].map((deg) => (
      <line
        key={deg}
    ...
    <circle cx="100" cy="100" r="3" fill={color} opacity="0.5" />
  </svg>
);
...
const WireframeFlow = ({ color }: { color: string }) => (
  <svg className="absolute inset-0 w-full h-full opacity-[0.07] pointer-events-none" viewBox="0 0 300 120" preserveAspectRatio="xMidYMid slice">
    {[0, 1, 2, 3, 4].map((i) => (
      <path
        key={i}
        d={`M0 ${20 + i * 20} C75 ${20 + i * 20 + (i % 2 === 0 ? -15 : 15)}, 225 ${20 + i * 20 + (i % 2 === 0 ? 15 : -15)}, 300 ${20 + i * 20}`}
        fill="none"
        stroke={color}
    ...
    {[60, 150, 240].map((x) => (
      <circle key={x} cx={x} cy="60" r="4" fill="none" stroke={color} strokeWidth="0.7" opacity="0.5" />
    ))}
  </svg>
);
...
const CardLabel = ({ children, color }: { children: React.ReactNode; color: string }) => (
  <div className={`hud-font-display text-[9px] font-600 tracking-[0.25em] uppercase ${color} flex items-center gap-1.5 mb-2`}>
...
const useLiveValue = (base: number, variance: number, interval = 800) => {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setValue(base + rand(-variance, variance));
    }, interval);
...
const LiquidityFlowCard = ({ bestPath }: { bestPath: BestPath | null }) => {
  ...
  return (
    <motion.div
      className="glass-card animate-glow-emerald relative overflow-hidden scanlines corner-tl corner-br"
      style={{ color: '#34d399' }}
    ...
    >
      ...
      <div className="relative z-10 p-4">
        <CardLabel color="text-emerald-400">
          <Waves size={10} /> Liquidity Flow
        </CardLabel>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            <div key={label} className="bg-emerald-400/5 rounded-md p-2 border border-emerald-400/10">
              ...
              <motion.div
                key={value}
                className="hud-font-display text-[13px] font-600 text-emerald-300"
...
const TransactionFeedCard = ({ transactions }: { transactions: BestPath[] }) => {
  return (
    ...
    >
      ...
      <div className="relative z-10 p-4">
        <CardLabel color="text-purple-400">
          <Activity size={10} /> Live Transactions
        </CardLabel>
          ...
          <AnimatePresence initial={false}>
            {transactions.map((tx) => (
              <motion.div
                ...
                animate="visible"
                exit="exit"
                className="flex items-center gap-2 py-1.5 px-2 rounded-md bg-purple-400/5 border border-purple-400/10 hover:bg-purple-400/10 transition-colors group"
              >
                {/* Type icon */}
...
const NetworkPulseCard = () => {
  ...
  return (
    <motion.div
      className="glass-card animate-glow-lemon relative overflow-hidden scanlines corner-tl corner-br"
      style={{ color: '#e8ff00' }}
    ...
    >
      ...
      <div className="relative z-10 p-4">
        <CardLabel color="text-yellow-300">
          <Cpu size={10} /> Network Pulse
        </CardLabel>
        ...
        <div className="flex items-end gap-[2px] h-12 mb-3">
          {bars.map((h, i) => (
            <motion.div
              key={i}
              className="flex-1 rounded-sm bg-gradient-to-t from-yellow-300/80 to-yellow-200/30"
              style={{ height: `${h}%` }}
              initial={{ height: '10%' }}
              animate={{ height: `${h}%` }}
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={label} className="text-center">
              ...
              <motion.div
                key={value}
                className="hud-font-display text-[11px] text-yellow-200"
                initial={{ opacity: 0.5 }}
                animate={{ opacity: 1 }}
...
const SecurityOracleCard = () => {
  ...
  return (
    <motion.div
      className="glass-card animate-glow-cyan relative overflow-hidden scanlines corner-tl corner-br"
      style={{ color: '#22d3ee' }}
      initial={{ opacity: 0, x: 20 }}
      animate={{ opacity: 1, x: 0 }}
    ...
    >
      ...
      <div className="relative z-10 p-4">
        <CardLabel color="text-cyan-400">
          <Shield size={10} /> Security Oracle
        </CardLabel>
          ...
          <div className="flex-1">
            ...
            <div className="h-1.5 bg-cyan-400/10 rounded-full overflow-hidden">
              <motion.div
                ...
                animate={{ width: `${Math.min(threatLevel, 100)}%` }}
...
export const TransactionHUD = ({ bestPath, transactions }: TransactionHUDProps) => {
  return (
    <>
      ...
      <div
        className="hud-root fixed inset-0 pointer-events-none"
        style={{ zIndex: 10 }}
        aria-label="Transaction monitoring HUD"
      >
        {/* Floating decorative icons layer */}
        <FloatingIcons />

        {/* Main layout: padding from edges, pointer-events restored on cards */}
