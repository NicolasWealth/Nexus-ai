...
const GLITCH_STYLES = `
  ...
  .glitch-active {
    animation: glitch-skew 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards;
    position: relative;
  }
  ...
  .glitch-active::before {
    color: #ff003c;
    animation: glitch-before 0.4s steps(1) forwards;
  }
  .glitch-active::after {
    color: #00ffff;
    animation: glitch-after 0.4s steps(1) forwards;
  }
  .glitch-text-active {
    animation: rgb-shift 0.4s steps(2) forwards;
  }
  .nexus-font { font-family: 'Orbitron', monospace; }
  .mono-font  { font-family: 'Share Tech Mono', monospace; }

  /* Scanline overlay on the hero section */
  .hero-scanline {
    position: absolute;
    left: 0; right: 0;
    height: 2px;
    background: linear-gradient(to right, transparent, rgba(52,211,153,0.15), transparent);
    pointer-events: none;
...
function App() {
  ...
  // Track previous glitchKey to detect new arrivals
  const prevGlitch = useState(glitchKey)[0]
  const isGlitching = glitchKey !== prevGlitch || glitchKey > 0
    ...
    <PrivacyShieldProvider>
      ...
      <div className="relative z-20 flex flex-col items-center min-h-screen">
        ...
        <section className="relative flex flex-col items-center justify-center min-h-screen w-full px-6 overflow-hidden">
          ...
          >
            <span
              className={`w-2 h-2 rounded-full ${connected ? 'conn-active' : 'conn-inactive'}`}
            />
          ...
          >
            ...
            <div className="relative w-28 h-28 rounded-full border border-emerald-400/20 bg-black/60 backdrop-blur-sm flex items-center justify-center overflow-hidden">
              <img src={reactLogo} className="w-10 h-10 opacity-70" alt="React" />
              <img src={viteLogo}  className="w-8  h-8  opacity-60 absolute bottom-4 right-4" alt="Vite" />
            </div>
          </motion.div>

          {/* Title with glitch effect keyed to glitchKey */}
          <AnimatePresence mode="wait">
            ...
            >
              <h1
                className={`nexus-font text-5xl md:text-6xl font-900 tracking-tight mb-2 text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-purple-400 to-yellow-300 ${isGlitching && glitchKey > 0 ? 'glitch-active' : ''}`}
                data-text="NEXUS AI"
              >
                NEXUS AI
              </h1>
              <p
                className={`mono-font text-[11px] tracking-[0.35em] text-emerald-400/70 uppercase ${isGlitching && glitchKey > 0 ? 'glitch-text-active' : ''}`}
              >
          ...
          >
            <p className="mono-font text-[12px] text-white/40 leading-relaxed">
              ...
              <span className="text-red-400/80">high cross-border fees</span> cost billions
              per year. Nexus AI deploys an autonomous agent that runs real-time graph
              pathfinding across{' '}
          ...
          >
            {[
              {
                label: 'ROUTES FOUND',
                value: <AnimatedCount value={transactions.length} />,
                color: 'text-emerald-400',
              },
              {
                label: 'AVG SAVINGS',
                value: bestPath ? `${bestPath.savings}%` : '—',
                color: 'text-purple-400',
              },
              {
                label: 'AVG FEE',
                value: bestPath ? `$${bestPath.totalFee.toFixed(3)}` : '—',
                color: 'text-yellow-300',
              },
            ].map(({ label, value, color }) => (
              <div
                key={label}
                className="flex flex-col items-center gap-1 px-3 py-3 rounded-lg border border-white/8 bg-black/40 backdrop-blur-sm"
              >
                <span className="mono-font text-[8px] text-white/30 tracking-widest">{label}</span>
                <span className={`nexus-font text-lg font-700 ${color}`}>{value}</span>
              </div>
            ))}
          </motion.div>

          {/* Best path display — glitches in on each new path */}
          <AnimatePresence mode="wait">
            {bestPath && (
              <motion.div
              ...
              >
                <div className="rounded-xl border border-emerald-400/20 bg-black/60 backdrop-blur-md p-4">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <span className="mono-font text-[9px] tracking-[0.2em] text-emerald-400/60">
                      OPTIMAL PATH FOUND
                    </span>
                  ...
                  <div className="flex items-center gap-1 flex-wrap mb-3">
                    {bestPath.route.map((hop, i) => (
                      <div key={i} className="flex items-center gap-1">
                        <span className="mono-font text-[9px] px-2 py-0.5 rounded bg-emerald-400/10 border border-emerald-400/20 text-emerald-300">
                          {hop.chain}
                        </span>
                        {i < bestPath.route.length - 1 && (
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      ...
                      <div key={label} className="text-center">
                        ...
                        <div className={`mono-font text-[10px] font-600 ${c}`}>{val}</div>
                      </div>
                    ))}
                  </div>
          ...
          >
            {[
              { label: 'React 19',      color: 'text-cyan-400   border-cyan-400/20   bg-cyan-400/5'    },
              ...
              <span
                key={label}
                className={`mono-font text-[9px] px-3 py-1 rounded-full border tracking-widest ${color}`}
              >
                {label}
              </span>
            ))}
        ...
        <div className="w-full border-y border-emerald-400/10 bg-black/50 backdrop-blur-sm py-2 overflow-hidden">
          <div className="stream-track flex gap-16 whitespace-nowrap">
            {[...STREAM_ITEMS, ...STREAM_ITEMS].map((item, i) => (
              <span key={i} className="mono-font text-[9px] text-emerald-400/40 tracking-[0.2em] uppercase">
                ◈ {item}
              </span>
            ))}
          </div>
